...
window.AUTO_LAB_DATA = {
 ...
 "telematics": {
  ...
  missions: [
    ...
    {id:"RF-04",risk:4,title:"텔레매틱스 원격 명령 무인증",r155:"R155 Annex5 · T16 원격 조작 (A16.1 원격키/텔레매틱스)",brief:"앱·서버를 거치는 원격 잠금해제/시동이 <b>소유자 인증을 제대로 하지 않으면</b>, 공격자가 원격으로 차량을 조작합니다.",try:"remote unlock → remote start",hint:"remote 결과가 \"성공(무인증)\"이면 취약, \"거부(인증 검증)\"면 양호.",why:"텔레매틱스 원격 명령이 소유자 인증 없이 전달됩니다. 원격 기능은 편의만큼 인증이 중요합니다.",ez:"집 현관을 여는 앱이 로그인·본인확인 없이 \"열어\" 버튼만 있으면 누구나 연다.",defense:"원격 명령마다 강한 소유자 인증(토큰·MFA)과 서버-차량 상호 인증, 명령 서명·재전송 방지를 적용한다.",cmds:["remote unlock", "remote start"],options:["원격 명령 성공 — 인증 없이 명령이 차량에 전달됨", "원격 명령 거부 — 소유자 인증·토큰 검증", "텔레매틱스 정보 없음", "네트워크 분리됨"],evidence:["원격 명령 성공 — 인증 없이 명령이 차량에 전달됨"],verdict:function(v){return v.telematics.authRemote?'good':'vuln';},fix:function(v){v.telematics.authRemote=true;},fixNote:"텔레매틱스 원격 명령에 소유자 인증·토큰 검증을 적용했습니다."}
  ]
};
