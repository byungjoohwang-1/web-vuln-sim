...
window.AUTO_LAB_DATA = {
 ...
 "privacy": {
  ...
  missions: [
    ...
    {id:"BE-02",risk:5,title:"OTA 위조 펌웨어 배포 (서명 검증 부재)",r155:"R155 Annex5 · T12 업데이트 훼손 (A12.1 OTA)",brief:"OTA 펌웨어에 <b>서명·무결성 검증</b>이 없으면, 공격자가 위조 펌웨어를 배포해 차량에 설치시킵니다. 전 차량 규모의 위협입니다.",try:"ota push evil.fw",hint:"ota push 결과가 \"성공(서명 없이 설치)\"이면 취약, \"거부(서명 검증)\"면 양호.",why:"OTA 펌웨어를 서명 검증 없이 설치해 위조 이미지가 통합니다. 업데이트 경로는 가장 강하게 지켜야 합니다.",ez:"누가 보냈는지 확인 안 하는 자동 업데이트는, 사칭한 사람이 보낸 것도 그대로 깔아 버린다.",defense:"OTA 이미지에 공급자 서명·무결성 검증(secure update)을 적용하고, 서명 키는 HSM 에 두며 버전 다운그레이드를 막는다.",cmds:["ota push evil.fw"],options:["OTA 위조 펌웨어 설치 성공 — 서명 검증 없음", "OTA 위조 펌웨어 거부 — 서명·무결성 검증", "OTA 정보 없음", "업데이트 서버 다운"],evidence:["OTA 위조 펌웨어 설치 성공 — 서명 검증 없음"],verdict:function(v){return v.ota.signed?'good':'vuln';},fix:function(v){v.ota.signed=true;},fixNote:"OTA 에 서명·무결성 검증을 적용했습니다."},
