...
window.AUTO_LAB_DATA = {
 ...
 "bt": {
  ...
  missions: [
    ...
    {id:"ECU-02",risk:4,title:"OBD 무보호 진단 접근(게이트웨이 미필터)",r155:"R155 Annex5 · T18 진단 접근 (A18.3 OBD 동글)",brief:"OBD 포트의 진단 요청이 게이트웨이에서 걸러지지 않고 <b>내부망으로 무제한 전달</b>되면, 동글 하나로 제어망까지 도달합니다.",try:"obd connect",hint:"obd 결과가 \"무제한 전달\"이면 취약, \"필터링·인증\"이면 양호.",why:"OBD 진단 접근을 게이트웨이가 통제하지 않아 물리 접근만으로 제어망에 닿습니다.",ez:"정비용 점검구가 사실은 건물 전체로 통하는 뒷문이면, 점검한다며 아무 데나 들어갈 수 있다.",defense:"OBD↔내부망 사이 게이트웨이가 진단 메시지를 필터링·인증하고, 위험 서비스는 보안 접근 뒤에만 연다.",cmds:["obd connect"],options:["OBD 진단 요청이 제어망으로 무제한 전달됨", "OBD 는 필터링·인증됨(위험 서비스는 보안 접근 뒤)", "OBD 포트 없음", "JTAG 잠김"],evidence:["OBD 진단 요청이 제어망으로 무제한 전달됨"],verdict:function(v){return v.obd.gatewayFiltered?'good':'vuln';},fix:function(v){v.obd.gatewayFiltered=true;},fixNote:"OBD 진단 접근에 게이트웨이 필터링·인증을 적용했습니다."},
    {id:"ECU-03",risk:5,title:"Secure Flash 부재 — 위조 펌웨어 기록",r155:"R155 Annex5 · T12 업데이트 훼손 (A12.2 로컬 플래시)",brief:"ECU 플래시에 <b>서명·무결성 검증</b>이 없으면, 위조 펌웨어를 그대로 기록해 차량을 공격자 의도대로 동작시킵니다.",try:"uds IVI flash",hint:"flash 결과가 \"성공(서명 없이 기록)\"이면 취약, \"실패(secure flash)\"면 양호.",why:"인포테인먼트 ECU 가 서명 검증 없이 펌웨어를 기록합니다. 위조 펌웨어가 그대로 실행됩니다.",ez:"누가 보낸 건지 확인 안 하고 받은 앱을 그대로 설치하는 것과 같다.",defense:"secure flash(플래시 시 서명·무결성 검증)와 secure boot(부팅 시 서명 검증)를 함께 적용하고, 서명 키는 HSM 에 둔다.",cmds:["uds IVI flash"],options:["위조 펌웨어 기록 성공 — 서명 검증 없이 플래시됨", "위조 펌웨어 거부 — secure flash 서명·무결성 검증", "ECU 를 찾을 수 없음", "보안 접근 필요"],evidence:["위조 펌웨어 기록 성공 — 서명 검증 없이 플래시됨"],verdict:function(v){var e=v.ecus.IVI;return (e.secureFlash||(e.secureBoot&&e.signedFw))?'good':'vuln';},fix:function(v){v.ecus.IVI.secureFlash=true;},fixNote:"인포테인먼트 ECU 에 secure flash 서명·무결성 검증을 적용했습니다."},
