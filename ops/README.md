...
파이프라인이 지금 무엇을 하는지 한 화면에서 실시간으로 본다. **관측 전용(read-only)** —
`state.db` 를 읽기 전용으로 열어 그리기만 하고 파이프라인을 조작하지 않는다.

```
python ops/dashboard.py            # 0.0.0.0:8787 서빙 (기본)
ops\run_dashboard.bat              # 더블클릭으로 실행(창 닫으면 종료)
```

- 같은 와이파이의 **스마트폰**에서 `http://<이-PC-LAN-IP>:8787` 로 접속하면 실시간으로 보인다.
  실행하면 콘솔에 phone 주소가 찍히고, 대시보드 하단 "접속 안내"에도 표시된다.
- 10초마다 `/api/state` 를 폴링해 부분 갱신(폰 데이터·배터리 절약). 수집 상태·7단계
  흐름·승인 대기·콘텐츠화 후보·실행 이력·오늘의 브리프를 보여준다.
- 자격증명·비밀은 `state.db` 에 없다(공개 CVE 피드 사실 + 진행 상태뿐). 그래도 인증이
  없으므로 **가정용 사설망 안에서만** 접속한다 — 라우터 포트포워딩 금지.
- 방화벽: python 인바운드가 활성 프로필에 허용돼 있어야 폰에서 닿는다(대개 이미 허용됨).
